        ...
        <div class='wrapper'>
            ...
            <div class="wdm_row">
                <div class='col_left'>Use the <a
                    ...
                    to manage multi-language search.
			...
			<?php if ( $is_plugin_active ) { ?>
				...
				foreach ( $solr_indexes as $solr_index_indice => $solr_index ) {
					?>
                    <div class="wdm_row wpsolr_index_<?php echo $loop; ?>">
                        <div class='col_left'>
                        ...
                        <div class='col_right'>
                            ...
                            Is indexing: &nbsp;
                            <select class="wpsolr_index_language_code"
								...
								foreach ( $languages as $language_code => $language ) {
									...
									echo sprintf( "<option value='%s' %s>%s</option>",
										$language_code,
										selected( $language_code, isset( $extension_options['solr_index_indice'][ $solr_index_indice ]['indexing_language_code'] )
											? $extension_options['solr_index_indice'][ $solr_index_indice ]['indexing_language_code']
											: '' ),
										$language_code );
                            ...
                            <input type='checkbox' class="wpsolr_index_is_default_search"
                                   name='<?php echo $extension_options_name; ?>[solr_index_indice][<?php echo $solr_index_indice ?>][is_default_search]'
                                   value='1'
								<?php
								checked( 1, isset( $extension_options['solr_index_indice'][ $solr_index_indice ]['is_default_search'] )
									? $extension_options['solr_index_indice'][ $solr_index_indice ]['is_default_search']
									: '' );
								?>
                            >
                ...
                <div class="submit">
					<?php if ( ! $license_manager->is_installed || $license_manager->get_license_is_activated( $package_name ) ) { ?>
                        <div class="wpsolr_premium_block_class">
							<?php echo $license_manager->show_premium_link( true, $package_name, OptionLicenses::TEXT_LICENSE_ACTIVATED, true ); ?>
                        </div>
                        <input
                                ...
                                value="Save Options"/>
					<?php } else { ?>
						<?php echo $license_manager->show_premium_link( true, $package_name, 'Save Options', true ); ?>
                        <br/>
					<?php } ?>
                </div>
            </div>
