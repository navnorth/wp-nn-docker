            ...
            <div class="wdm_note">
                ...
                <ul class="wdm_row">
					...
					<?php foreach ( $model_types as $model_type ) {
						$count_nb_documents_to_be_indexed = $solr->get_count_nb_documents_to_be_indexed( $model_type );
                        ...
                        <li>
							...
							<?php if ( empty( $count_nb_documents_to_be_indexed ) ) { ?>
								...
								<?php echo sprintf( 'All %s are indexed', $model_type->get_label() ); ?>.
							<?php }
							if ( $count_nb_documents_to_be_indexed > 0 ) { ?>
                                <img src="<?php echo plugins_url( 'images/warning.png', WPSOLR_PLUGIN_FILE ); ?>"
                                     class="wpsolr_premium_text_class" style="display:inline">
								<?php echo sprintf( '%s %s', $count_nb_documents_to_be_indexed, $model_type->get_label() ); ?>
                                not indexed yet. Click on the
                                button "synchronize" to
                                index
                                them.
							<?php }
							if ( ( $model_type instanceof WPSOLR_Model_Type_Post ) && ( $count_blacklisted_ids > 0 ) ) {
								?>
							...
							if ( ! empty( $locked_post_types[ $model_type->get_type() ] )
								...
								if ( ( $current_user instanceof WP_User ) && ( $process_label !== $current_user->user_email ) ) {
                                    ...
                                    <div style="display: inline-block">
                                        <input type="button"
                                               data-wpsolr-post_type="<?php echo $model_type->get_type(); ?>"
