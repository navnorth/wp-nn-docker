...
namespace wpsol\pro\includes\readmetxt;
...
/**
 * Extract and format sections from a readme.txt
 * @link https://github.com/markjaquith/WordPress-Plugin-Readme-Parser
 */
Class WPSOLRPro_Readme_Txt {
	...
	function parse_readme_contents( $file_contents ) {
		...
		// version
		if ( preg_match( '|current version:(.*)|i', $file_contents, $_version ) ) {
			$version = $this->sanitize_text( $_version[1] );
		} else {
			$version = null;
		}

		// author
		if ( preg_match( '|author:(.*)|i', $file_contents, $_author ) ) {
			$author = $this->sanitize_text( $_author[1] );
		} else {
			$author = null;
		}

		// homepage
		if ( preg_match( '|Author URI:(.*)|i', $file_contents, $_homepage ) ) {
			$homepage = $this->sanitize_text( $_homepage[1] );
		} else {
			$homepage = null;
		}
