...
function wpsolr_admin_init() {
	...
	register_setting( 'extension_theme_jobify_opt', WPSOLR_Option::OPTION_THEME_JOBIFY );
	register_setting( 'extension_theme_listable_opt', WPSOLR_Option::OPTION_THEME_LISTABLE );
	register_setting( 'extension_theme_directory2_opt', WPSOLR_Option::OPTION_THEME_DIRECTORY2 );
...
function fun_add_solr_settings() {
	...
	if ( false !== strpos( $_SERVER['REQUEST_URI'], 'solr_settings' ) ) {
		...
		// Dates utility
		wp_enqueue_script( 'moment', plugins_url( '../js/moment/moment-with-locales.min.js', __FILE__ ), [], WPSOLR_PLUGIN_VERSION );

		// bcrypt utilities. https://github.com/dcodeIO/bcrypt.js
		wp_enqueue_script( 'bcryptjs', plugins_url( '../js/bcryptjs/bcrypt.min.js', __FILE__ ), [ 'jquery' ], WPSOLR_PLUGIN_VERSION );
		...
		wp_enqueue_style( 'select2', plugins_url( '../../core/css/select2.css', __FILE__ ) );
...
function fun_set_solr_options() {
    ...
    <div class="wdm-wrap">
        ...
        <input type='hidden' id='adm_path'
               value='<?php echo apply_filters( WPSOLR_Events::WPSOLR_FILTER_UAT_TEST_ADMIN_URL, admin_url() ); ?>'>
        <!-- for ajax -->
...
function wpsolr_admin_sub_tabs( $subtabs, $before = null ) {
	...
	echo '<div id="icon-themes" class="icon32"><br></div>';
	echo '<div class="nav-tab-wrapper wdm-vertical-tabs wpsolr-col-3"><ul>';
	...
	foreach ( $subtabs as $subtab_indice => $subtab ) {
		...
		if ( false === strpos( $name, 'wpsolr_premium_class' ) ) {
			echo "<li><a class='nav-tab$class $extra_class' href='admin.php?page=solr_settings&tab=$tab&subtab=$subtab_indice'>$name</a></li>";
		} else {
			echo $name;
		}
