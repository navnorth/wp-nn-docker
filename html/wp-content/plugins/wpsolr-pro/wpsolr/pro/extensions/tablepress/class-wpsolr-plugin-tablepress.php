...
namespace wpsolr\pro\extensions\tablepress;
...
/**
 * Class WPSOLR_Plugin_TablePress
 * @package wpsolr\pro\extensions\tablepress
 *
 * Manage TablePress
 * @link https://wordpress.org/plugins/tablepress/
 */
class WPSOLR_Plugin_TablePress extends WpSolrExtensions {
	...
	function __construct() {
		...
		if ( is_admin() && WPSOLR_Service_Container::getOption()->get_tablepress_is_index_shortcodes() ) {
			// This extension must only execute in admin, and only when authorized.

			add_filter( 'tablepress_table_output', [
				$this,
	...
	public function tablepress_table_output( $output, $table, $render_options ) {
		...
		$result = $this->implode_recursive( WPSOLR_IndexSolariumClient::CONTENT_SEPARATOR, $table['data'] );
