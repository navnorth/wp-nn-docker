...
class WPSOLR_Option_Indexes extends WpSolrExtensions {
	...
	public function build_config( &$solr_index_indice = null, $language_code = null, $timeout ) {
		...
		$index_hosting_api_id = isset( $solr_index['index_hosting_api_id'] ) ? $solr_index['index_hosting_api_id'] : WPSOLR_Hosting_Api_None::HOSTING_API_ID;
		$index_region_id      = isset( $solr_index['index_region_id'] ) ? $solr_index['index_region_id'] : '';
		...
		$config = [
			...
			'index_label'  => $label,
			'scheme'       => $solr_index['index_protocol'], //$hosting_api->get_data_by_id( WPSOLR_Hosting_Api_Abstract::DATA_SCHEME, $index_region_id, $solr_index['index_protocol'] ),
			...
			'port'         => $solr_index['index_port'] , //$hosting_api->get_data_by_id( WPSOLR_Hosting_Api_Abstract::DATA_PORT, 'donotcare', $solr_index['index_port'] ),
			'path'         => $index_path, //$hosting_api->get_data_by_id( WPSOLR_Hosting_Api_Abstract::DATA_PATH, $label, $index_path ),
			'timeout'      => $timeout,
		];
