...
class OptionLicenses extends WpSolrExtensions {
	...
	function get_license_is_need_verification( $license_type ) {
		...
		return isset( $licenses[ $license_type ] )
		       && isset( $licenses[ $license_type ][ self::FIELD_IS_ACTIVATED ] )//&& isset( $licenses[ $license_type ][ self::FIELD_NEEDS_VERIFICATION ] )
			;
	}
	...
	static function get_license_types() {
		...
		return [
			...
			self::LICENSE_PACKAGE_JOBIFY                            => [
				self::LICENSE_EXTENSION                => WpSolrExtensions::EXTENSION_THEME_JOBIFY,
			...
			self::LICENSE_PACKAGE_LISTABLE                          => [
				self::LICENSE_EXTENSION                => WpSolrExtensions::EXTENSION_THEME_LISTABLE,
			...
			self::LICENSE_PACKAGE_DIRECTORY2                        => [
				self::LICENSE_EXTENSION                => WpSolrExtensions::EXTENSION_THEME_DIRECTORY2,
	...
	static public function get_license_activation_infos() {
		...
		if ( ! empty( $licenses ) ) {
			foreach ( $licenses as $license ) {
				...
				if ( ! empty( $license[ self::FIELD_LICENSE_EXPIRATION_DATE ] ) && ! empty( $license[ self::FIELD_LICENSE_HASH ] ) ) {
					$result[ self::FIELD_LICENSE_EXPIRATION_DATE ] = $license[ self::FIELD_LICENSE_EXPIRATION_DATE ];
					$result[ self::FIELD_LICENSE_HASH ]            = $license[ self::FIELD_LICENSE_HASH ];
					$result[ self::FIELD_LICENSE_DOMAIN ]          = home_url();
	...
	static function ajax_activate_licence() {
		...
		if ( isset( $response_object ) && OptionManagedSolrServer::is_response_ok( $response_object ) ) {
			...
			$licenses[ $license_package ] = [
				...
				self::FIELD_LICENSE_EXPIRATION_DATE     => OptionManagedSolrServer::get_response_result( $response_object, self::LICENSE_EXPIRATION_DATE ),
				self::FIELD_LICENSE_HASH                => OptionManagedSolrServer::get_response_result( $response_object, self::LICENSE_HASH ),
			];
	...
	static function ajax_verify_licence() {
		...
		if ( isset( $response_object ) && OptionManagedSolrServer::is_response_ok( $response_object ) ) {
			...
			$activated_url = OptionManagedSolrServer::get_response_result( $response_object, self::FIELD_LICENSE_DOMAIN );
			...
			if ( isset( $licenses[ $license_package ] ) ) {
				...
				foreach ( $licenses as $license_package_installed => $license ) {
					...
					if ( $license_activation_uuid === $license[ self::FIELD_LICENSE_ACTIVATION_UUID ] ) {
						...
						// Add license infos
						$licenses[ $license_package_installed ][ self::FIELD_LICENSE_EXPIRATION_DATE ] = OptionManagedSolrServer::get_response_result( $response_object, self::LICENSE_EXPIRATION_DATE );
						$licenses[ $license_package_installed ][ self::FIELD_LICENSE_HASH ]            = OptionManagedSolrServer::get_response_result( $response_object, self::LICENSE_HASH );
					}
				}
				self::set_option_data( self::OPTION_LICENSES, $licenses );
			}
