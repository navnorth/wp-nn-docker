...
class WPSOLR_SearchElasticaClient extends WPSOLR_AbstractSearchClient {
	...
	protected function admin_is_index_exists( $is_throw_error = false ) {
		...
		if ( true ) {
			...
			try {
				...
				$stats = $this->get_elastica_index()->getStats();
			...
			if ( ! empty( $data ) && ! empty( $data['Message'] ) ) {
				...
				throw new \Exception( "Problem while connecting to your index :<br><br> \"{$data['Message']}\"" );
			}
	...
	function search_engine_client_add_filter_not_in_terms_of_other_sites(
		...
		$terms = new \Elastica\Query\Terms();
		$terms->setTerms( $field_name, $field_values );
		$terms_not = new \Elastica\Query\BoolQuery();
		$terms_not->addMustNot( $terms );
		...
		$this->query_filters[ $field_name ]->addShould( [ $terms_not, $terms_site_id ] );
	}
	...
	public function search_engine_client_add_filter_exists( $filter_name, $field_name ) {
		...
		if ( ! isset( $this->query_filters[ $field_name ] ) ) {
			// Add 'exists'
			$this->query_filters[ $field_name ] = $this->search_engine_client_create_filter_exists( $field_name );
		}
	...
	public function search_engine_client_create_filter_exists( $field_name ) {
		...
		return ( new \Elastica\Query\BoolQuery() )->addMust( new Exists( $field_name ) );
	}
	 ...
	 * @return WPSOLR_ResultsElasticaClient
	 */
	public function search_engine_client_get_suggestions_keywords( $query ) {
