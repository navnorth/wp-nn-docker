...
abstract class WPSOLR_AbstractSearchClient extends WPSOLR_AbstractEngineClient {
	 ...
	 */
	abstract public function search_engine_client_get_suggestions_keywords( $query );
	 ...
	 *
	 */
	private function add_ids_excluded_from_search_filter_query_fields( WPSOLR_Query $wpsolr_query ) {
	...
	function add_filter_query_fields(
		...
		if ( ! is_admin() ) {
			...
			$posts_to_filter_out = apply_filters( WPSOLR_Events::WPSOLR_FILTER_POST_STATUSES_TO_FILTER_OUT, self::$default_status_to_filter_out );
	 ...
	 * @return string
	 */
	abstract public function search_engine_client_escape_term( $keywords );
	...
	function set_keywords(
		...
		$this->add_field_keywords( $keywords );
	 ...
	 */
	protected function add_field_keywords( &$main_keywords ) {

		$results = [];

		$field_keywords = apply_filters( WPSOLR_Events::WPSOLR_FILTER_QUERY_ADD_EXTRA_FIELD_QUERIES, [] );

		foreach ( $field_keywords as $field_name => $keyword ) {

			if ( ! empty( trim( $keyword ) ) ) {
				$results[] = sprintf( '%s:(%s)', $field_name . WpSolrSchema::_SOLR_DYNAMIC_TYPE_TEXT, $this->search_engine_client_escape_term( $keyword ) );
			}
		}
